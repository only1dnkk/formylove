...
// Навигация
// ================================
const defaultOrder = ['intro', 'intro2', 'intro3', 'quest1', 'greeting'];
...
// ================================
// Финальный текст — печать по буквам
// ================================
const finalText = `С днём рождения тебя! Пусть этот год принесёт тебе много радости, вдохновения и приятных моментов.
...
Твои глаза всегда поднимают настроение, а твоя доброта чувствуется сразу.

Твои глаза невероятно красивы, порой хочется смотреть в них вечность
...
function revealTextByLetters(el, text, baseDelay = 18, randomSpan = 22){
  ...
  for (let i=0; i<trimmed.length; i++){
    ...
    s.className = 'char';
    s.textContent = (ch === ' ') ? '\u00A0' : ch; // фикс пробелов
...
let redeemed = 0;             // зачисленные сердечки (после «Забрать»)
let heartsNodes = [];
let pendingHeart = null;      // сердечко, по которому кликнули (ждёт «Забрать»)
...
function updateCounter(){
  if (foundCountEl) foundCountEl.textContent = String(Math.min(redeemed, 5));
  if (btnToGreeting) btnToGreeting.disabled = redeemed < 5;
}

function clearHearts(){ heartsNodes.forEach(n => n.remove()); heartsNodes = []; }
...
function spawnHearts(n=5){
  ...
  for(let i=0; i<n; i++){
    ...
    heart.addEventListener('click', (e)=>{
      e.stopPropagation();
      if (redeemed >= 5) { showToast('Все 5 фактов уже забраны 💗'); return; }

      // визуально пометим выбранное сердце
      pendingHeart = heart;
      heart.style.transition = 'transform .2s var(--ease), opacity .2s var(--ease)';
      heart.style.transform = 'translateY(-6px) rotate(-45deg) scale(1.08)';
      heart.style.opacity = '0.9';
      // показываем модалку с текущим фактом (по номеру redeemed)
      const idx = Math.min(redeemed, coupons.length - 1);
      showCoupon(coupons[idx].title, coupons[idx].text);
    }, { once:false }); // можно нажать повторно, если модалка закрывалась
...
if (btnSpawnAgain) btnSpawnAgain.addEventListener('click', (e)=>{ makeRipple(e); spawnHearts(7); });
if (btnToGreeting) btnToGreeting.addEventListener('click', (e)=>{ makeRipple(e); showPanel('greeting'); });

// ================================
// Модалка купона + полёт сердечка в счётчик
// ================================
const modal = (() => {
  ...
  const btn = document.createElement('button'); btn.className = 'btn'; btn.textContent = 'Забрать';
  card.append(h4, p, btn); m.append(card); document.body.appendChild(m);

  btn.addEventListener('click', ()=>{
    const startRect = card.getBoundingClientRect(); // до скрытия
    m.classList.remove('show');
    animateHeartToCounter(startRect, () => {
      // после долёта: зачисляем и чистим выбранное сердечко
      if (pendingHeart) {
        pendingHeart.style.transform = 'translateY(-8px) rotate(-45deg) scale(0.1)';
        pendingHeart.style.opacity = '0';
        setTimeout(()=> pendingHeart?.remove(), 260);
        pendingHeart = null;
      }
      redeemed = Math.min(redeemed + 1, 5);
      updateCounter();
      if (redeemed >= 5) showToast('Готово! Все 5 фактов забраны 💗');
    });
  });
...
function showCoupon(title, text){ modal.show(title, text); }
...
function animateHeartToCounter(startRect, onDone){
  ...
  anim.onfinish = () => {
    ...
    // вспышка сердца внутри плашки (за текстом)
...
// Всплеск по клику (кроме UI)
window.addEventListener('pointerdown', (e)=>{
  if (e.target.closest('.music, .btn, .chip, .slot, .closeMemories, .modal')) return;
